...
- 后端 course 域 `/api/series` 排序白名单（router.py L37 pattern + series_repo.py `_SORT_MAP`）实为：`default | newest | price_asc | price_desc`。**无 `popular`/`hot` 排序**。
...
- **语义澄清**：本系统"章节"指 `session_video_chapter`（视频内 start_second/end_second 分节），**非"学习课次"**。课次（session）内容端点是真实的（`/api/cohorts/{id}/modules` + admin session CRUD）；"章节（视频分节）"则未单独暴露 CRUD 端点。
...
### 3.3 task57 结论
- **情况 B**：后端无独立章节（session_video_chapter）CRUD 端点 → **不硬造前端猜端点**，保持现状不动。
