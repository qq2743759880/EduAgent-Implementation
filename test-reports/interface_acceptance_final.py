...
def write_report(rows, missing, unused, front_norm, backend_norm):
    ...
    A("## 5. 发现的真实缺陷")
    A("")
    A("### P1 — HTTP 500（代码缺陷，随本次实跑 fail500 动态判定）")
    A("")
    if fail500 > 0:
        A("| 接口 | 期望 | 实际 | 根因线索 |")
        A("|---|---|---|---|")
    ...
    if fail500 > 0:
        A(f"- **验收结论：不通过**（存在 {fail500} 个 HTTP 500 真实缺陷 + 前端引用缺失端点 + DEBUG 鉴权降级部署风险）")
        A("")
        A("- **阻断项（必须先修）**：")
        ...
        A("- **验收结论：HTTP 500 阻断项已清零**（历史 6 个 500 缺陷经 2026-09-05 复跑 + 定向深探确认已修复）。剩余阻断/待办（非 500）：")
        A("  1. series DELETE 假删除（响应 200 但仅软下线 off_sale，记录仍在表）——需复核是否按 C5 软删契约落地")
        A("  2. DEBUG 鉴权降级部署风险（对外部署必须 DEBUG=False）")
        A("  3. 前端引用的 14 个端点后端缺失（见 §2/§4 missing_in_backend）")
