...
/* 0. 积分基线：发帖前记录，后续断言相对增量（脚本可重复执行，不依赖全新账号） */
const basePointsResp = await req("GET", "/api/gamification/me/points?page=1&page_size=20", { token });
const basePoints = typeof basePointsResp.total_points === "number" ? basePointsResp.total_points : 0;
console.log(`[基线] 发帖前 total_points=${basePoints}`);
...
check(
  "积分 = 基线 + 7（发帖+5 回帖+2）",
  points.total_points === basePoints + 7,
  `基线 ${basePoints}，实际 ${points.total_points}（期望 ${basePoints + 7}）`,
);
