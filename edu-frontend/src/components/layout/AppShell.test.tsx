 ...
 *
 * 覆盖：渲染（品牌/菜单 label）、激活态（matchPrefix 语义）、抽屉交互（<768 打开 /
 * Escape 关闭 / ≥768 隐藏移动按钮）、a11y（关闭态 inert）。
 *
...
describe("AppShell 激活态（matchPrefix 语义）", () => {
  it("pathname /courses/123 → 课程中心（matchPrefix=/courses）aria-current=page，其他项无", () => {
    ...
    const courses = within(sidebar).getByRole("link", { name: "课程中心" });
    const dashboard = within(sidebar).getByRole("link", { name: "学习仪表盘" });
    expect(courses).toHaveAttribute("aria-current", "page");
    expect(dashboard).not.toHaveAttribute("aria-current");
    // 品牌链接不参与激活
    const brand = within(sidebar).getByRole("link", { name: /EduAgent/ });
  ...
  it("pathname 精确命中（/dashboard）→ 无 matchPrefix 项按 === 激活", () => {
    ...
    const sidebar = getSidebar();
    expect(within(sidebar).getByRole("link", { name: "学习仪表盘" })).toHaveAttribute(
      "aria-current",
      "page",
    );
    expect(within(sidebar).getByRole("link", { name: "课程中心" })).not.toHaveAttribute(
      "aria-current",
    );
  });
});
...
describe("AppShell 移动抽屉（<768px）", () => {
  it("点击打开按钮 → 侧边栏 translate-x-0 + aria-expanded=true + 聚焦首菜单项", () => {
    ...
    expect(screen.getByRole("link", { name: "课程中心" })).toHaveFocus();
  });
...
describe("AppShell 桌面（≥768px）", () => {
  it("移动打开按钮以 md:hidden 隐藏（SSR/hydration 安全保留在 DOM）；侧边栏不 inert", () => {
    mockMatchMedia(true);
    renderShell();
