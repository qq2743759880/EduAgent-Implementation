...
describe("ServerTable", () => {
  ...
  it("点击「发现工具」→ 拉取 DB 工具列表并打开弹窗", async () => {
    mockGet.mockResolvedValueOnce({
      ...
      page: 1,
      page_size: 20,
      items: [{ id: 2, server_id: 1, tool_name: "add", display_name: "整数加法", description: "a+b", input_schema: { type: "object", properties: {} }, category: "data", yn: 1, discovered_at: "" }],
    });
    renderTable();
  ...
  it("注销 Server：DELETE /servers/{id}", async () => {
    mockDelete.mockResolvedValueOnce({ ok: true, server_id: 2, deleted: true });
    renderTable();
