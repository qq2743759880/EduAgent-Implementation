...
describe("RebuildDialog", () => {
  ...
  it("提交重建：POST /collections/rebuild body {collection_name, partition_name, mode} → 展示 job_id", async () => {
    mockPost.mockResolvedValueOnce({ job_id: "r_abc123def456", accepted: true, message: "已接受重建", estimated_rows: 4969 });
    renderDialog();
  ...
  it("切换全量模式后提交 mode=full", async () => {
    mockPost.mockResolvedValueOnce({ job_id: "r_x", accepted: true, message: "ok", estimated_rows: 0 });
    renderDialog();
