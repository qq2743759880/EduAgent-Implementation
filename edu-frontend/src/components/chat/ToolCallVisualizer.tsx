...
const STATUS_TONE: Record<MCPToolCallStatus, {
  ...
  timeout: {
    badge: "destructive",
    label: "超时",
    progress: "[&_[data-slot=progress-indicator]]:bg-destructive/70",
    icon: XCircle,
...
export function ToolCallVisualizer({
  ...
  const argsVal = React.useMemo(() => maybeParseJson(call.args_summary), [call.args_summary]);
  const argsStr = React.useMemo(
    () => (argsVal == null ? "—" : typeof argsVal === "string" ? argsVal : jsonStringify(argsVal)),
    [argsVal],
  );
  const resultVal = React.useMemo(() => maybeParseJson(call.result_summary), [call.result_summary]);
