...
export interface UseChatSessionsHandle {
  ...
  createAndSelect: (body?: { title?: string | null }) => Promise<ChatSession>;
  deleteSession: (id: string, nextId?: string | null) => Promise<void>;
  /** 保证存在一个 session（若未选中则新建）；返回 id */
  ensureSession: (body?: { title?: string | null }) => Promise<string | null>;
  refetch: () => void;
}
...
function writeLastId(id: string | null) {
  try {
...
export function useChatSessions(opts: UseChatSessionsOptions = {}): UseChatSessionsHandle {
  ...
  const effectiveSelectedId = useMemo<string | null>(() => {
    ...
    return data.some((s) => s.session_id === selectedId)
      ? selectedId
  ...
  const createMut = useMutation({
    mutationFn: (body: { title?: string | null } = {}) => createChatSession(body),
    onMutate: async (body) => {
  ...
  const deleteMut = useMutation({
    ...
    onMutate: async (id) => {
      await queryClient.cancelQueries({ queryKey: qk, exact: true });
      const prev = queryClient.getQueryData<ChatSession[]>(qk) ?? [];
      queryClient.setQueryData<ChatSession[]>(qk, prev.filter((s) => s.session_id !== id));
      return { prev };
    },
    onSuccess: (_resp, id, ctx) => {
      ...
      setSelectedId((prev) => {
        if (prev === id) {
          const remain = (ctx?.prev ?? []).filter((s) => s.session_id !== id);
          const next = remain[0]?.session_id ?? null;
          writeLastId(next);
          return next;
        }
        return prev;
  ...
  const ensureSession = useCallback<UseChatSessionsHandle["ensureSession"]>(async (body) => {
    ...
    if (current != null) {
      ...
      if (list.some((s) => s.session_id === current)) return current;
    }
