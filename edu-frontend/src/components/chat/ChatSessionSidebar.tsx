...
export interface ChatSessionSidebarProps {
  ...
  onPicked?: (sessionId: string | null) => void;
...
function displayPreview(s: ChatSession): string {
  const t = s.last_message_at ?? s.updated_at ?? s.created_at;
  ...
  if (d && Number.isFinite(d.getTime())) {
    ...
    return `最后消息 ${mm}-${dd} ${hh}:${mi}`;
  }
  return `${s.message_count ?? 0} 条消息`;
}
...
export function ChatSessionSidebar({
  ...
  const [selected, setSelected] = React.useState<string | null>(initialSessionId ?? storeSelected ?? null);
  React.useEffect(() => {
  ...
  const handleDelete = async () => {
    ...
    try {
      // 删除后自动切换临近的 session
      const idx = sessions.findIndex((s) => s.session_id === confirmDelete.session_id);
      const neighbor = sessions[idx + 1] ?? sessions[idx - 1] ?? null;
      await deleteSession(confirmDelete.session_id, neighbor?.session_id ?? null);
      setSelected(neighbor?.session_id ?? null);
      if (selected === confirmDelete.session_id) {
        onPicked?.(neighbor?.session_id ?? null);
      }
      toast.success("对话已删除");
