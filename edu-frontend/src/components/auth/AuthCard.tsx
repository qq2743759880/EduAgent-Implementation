    ...
    >
      <Card className="w-full max-w-md shadow-xl border-slate-200/80 bg-white/90 backdrop-blur">
        <CardHeader className="space-y-2 text-center pb-2">
          ...
          <CardTitle className="text-2xl font-bold text-slate-900 pt-2">{title}</CardTitle>
          {subtitle ? (
            <CardDescription className="text-sm text-slate-500">{subtitle}</CardDescription>
          ) : null}
        </CardHeader>
        <CardContent className="pt-4">{children}</CardContent>
        <CardFooter className="pt-0 pb-6 flex items-center justify-center gap-1 text-sm text-slate-600">
