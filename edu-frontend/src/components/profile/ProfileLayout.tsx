...
export function ProfileLayout({
  ...
  return (
    <div
      className={cn(
        "min-h-[calc(100vh-4rem)] w-full bg-gradient-to-br from-indigo-50/60 via-white to-sky-50/60 px-4 py-8 sm:px-6 lg:px-10",
        className,
      )}
    >
      <div className="mx-auto w-full max-w-6xl grid grid-cols-1 md:grid-cols-[240px_1fr] gap-6">
        {/* 左：用户简卡 */}
        <aside>
          <Card className="shadow-md border-slate-200/80 sticky top-6">
            <CardContent className="pt-8 pb-6 flex flex-col items-center text-center space-y-4">
              <div className="relative">
                <Avatar className="h-20 w-20 border-4 border-white shadow">
                  ...
                  <AvatarFallback className="bg-gradient-to-br from-indigo-600 to-sky-600 text-white text-2xl font-semibold">
                    {firstChar(displayName)}
                  </AvatarFallback>
                </Avatar>
                <span className="absolute -bottom-1 -right-1 inline-flex items-center justify-center h-7 w-7 rounded-full bg-amber-400 text-white shadow ring-2 ring-white">
                  <Sparkles className="h-3.5 w-3.5" />
                </span>
              </div>
              <div className="space-y-1">
                <h2 className="text-lg font-semibold text-slate-900">{displayName}</h2>
                <p className="text-xs text-slate-500 break-all px-2">{email || "尚未设置邮箱"}</p>
              </div>
              <Separator />
              <div className="w-full space-y-4 pt-1">
                ...
                <div className="space-y-2 text-left">
                  <div className="text-xs text-slate-500 px-1">学科偏好</div>
                  <div className="flex flex-wrap gap-1.5 min-h-[2rem] px-1">
                    {subjectPreferences.length === 0 ? (
                      <span className="text-xs text-slate-400">暂未设置，去「偏好设置」添加</span>
                    ) : (
                      subjectPreferences.map((k) => (
                        <Badge key={k} variant="secondary" className="bg-indigo-50 text-indigo-700 hover:bg-indigo-100 border border-indigo-100">
                          {SUBJECT_LABELS[k]}
                        </Badge>
                      ))
                    )}
