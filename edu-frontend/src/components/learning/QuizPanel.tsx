...
export function QuizPanel(props: QuizPanelProps) {
  ...
  useEffect(() => {
    setAnswer(useInitialAnswer(question));
    setUserAnalysis("");
    setUsedHint(false);
    setSubmitted(null);
    loadedAtRef.current = Date.now();
  ...
  return (
    <div className="space-y-4">
      <header className="flex flex-wrap items-center gap-2">
        ...
        {q.isError ? (
          <Badge variant="destructive" className="text-xs">出题接口异常（稍后重试）</Badge>
        ) : null}
              ...
              <div className="space-y-3">
                ...
                <div className="flex items-center justify-between">
                  <div className="text-xs text-muted-foreground">
                    当前用时 {(Math.max(1, Math.floor((Date.now() - loadedAtRef.current) / 1000)))} 秒
                  </div>
                  <Button
...
function StemBlock({ stem, hint }: { stem: string; hint: string | null }) {
  return (
    <div className="space-y-3">
      <div className="text-[15px] leading-7 whitespace-pre-wrap">{stem}</div>
      {hint && (
            ...
            return (
              <label
                ...
                className={cn(
                  ...
                  isPick && submitted && (submitted.is_correct ? "border-emerald-400 bg-emerald-50" : "border-rose-400 bg-rose-50"),
                  isRight && !isPick && submitted && "border-emerald-300 bg-emerald-50/60",
                )}
              >
                <RadioGroupItem value={o.key} id={`q-single-${o.key}`} className="mt-0.5" />
                <div className="flex-1">
                  <div className="text-sm">
                    <span className="mr-2 inline-flex h-5 w-5 items-center justify-center rounded-full bg-slate-100 text-xs font-semibold text-slate-700">
                      {o.key.toUpperCase()}
                    </span>
                    {o.text}
                  </div>
            ...
            return (
              <div
                ...
                className={cn(
                  ...
                  submitted && !checked && isRight && "border-emerald-300 bg-emerald-50/60",
                )}
              >
                <Checkbox
                  disabled={disabled}
            ...
            return (
              <label
                ...
                className={cn(
                  ...
                  selected && submitted &&
                    (submitted.is_correct ? "border-emerald-400 bg-emerald-50" : "border-rose-400 bg-rose-50"),
                  right && !selected && submitted && "border-emerald-300 bg-emerald-50/60",
                )}
              >
            ...
            <div key={i} className="space-y-1">
              <Label className="text-xs text-muted-foreground">
                ...
                {submitted && correctArr[i] != null ? (
                  <span className="ml-2 text-emerald-600">参考答案：{String(correctArr[i])}</span>
                ) : null}
              </Label>
              <Input
                disabled={disabled}
  ...
  return (
    <div
      className={cn(
        ...
        result.is_correct ? "border-emerald-300 bg-emerald-50/60" : "border-rose-300 bg-rose-50/60",
      )}
    >
        ...
        <div className="flex items-center gap-2">
          {result.is_correct ? (
            <>
              ...
              <div className="font-semibold text-emerald-700">回答正确 🎉</div>
            </>
          ) : (
            <>
              <XCircle className="h-5 w-5 text-rose-600" />
              <div className="font-semibold text-rose-700">回答错误</div>
            </>
          )}
      ...
      {analysis && (
        <div className="mt-3 rounded-lg bg-white/70 p-3 text-xs text-slate-700">
          <div className="font-semibold text-slate-700">我的思路：</div>
          <div className="mt-1 whitespace-pre-wrap">{analysis}</div>
        </div>
      )}

      {result.explain_content && (
        <div className="mt-3 rounded-lg bg-white/70 p-3 text-sm leading-6">
          <div className="mb-1 text-xs font-semibold text-slate-700">AI 解析：</div>
  ...
  return (
    <Card>
      <CardContent className="grid gap-4 p-5 md:grid-cols-[auto_minmax(0,1fr)] md:items-start">
        <div className="mx-auto grid h-12 w-12 place-items-center rounded-2xl bg-primary/10 text-primary md:mx-0">
        ...
        <div className="space-y-2">
          <div className="flex flex-wrap items-center gap-2">
            ...
            {question.is_from_wrong_book && (
              <Badge className="bg-rose-100 text-rose-700 hover:bg-rose-100 text-xs">来自错题本</Badge>
            )}
          </div>
...
function EmptyState({ onRetry }: { onRetry: () => void }) {
  return (
    <div className="rounded-2xl border border-dashed p-10 text-center">
      ...
      <div className="text-base font-semibold">暂时没有可用的习题</div>
  ...
  return (
    <div className="space-y-3 rounded-2xl border p-5">
      ...
      <div className="mt-4 space-y-2">
        <div className="h-11 w-full animate-pulse rounded-xl border bg-slate-100" />
        <div className="h-11 w-full animate-pulse rounded-xl border bg-slate-100" />
        <div className="h-11 w-1/2 animate-pulse rounded-xl border bg-slate-100" />
      </div>
    </div>
  );
}
