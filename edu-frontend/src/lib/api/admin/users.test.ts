/**
 * lib/api/admin/users.ts 单测（task03）
...
describe("角色切换 / 状态切换", () => {
  it("changeUserRole POST /{id}/role body {target_role}", async () => {
    mockPost.mockResolvedValueOnce({ data: { updated: true, user_id: 5, target_role: "teacher" } });
  ...
  it("changeUserRole 携带 reason", async () => {
    mockPost.mockResolvedValueOnce({ data: { updated: true, user_id: 5, target_role: "student" } });
  ...
  it("changeUserStatus POST /{id}/status body {status, yn}", async () => {
    mockPost.mockResolvedValueOnce({ data: { updated: true, user_id: 5, status: 0, yn: 0 } });
