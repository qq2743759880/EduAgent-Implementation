/**
 * lib/api/admin/rag.ts 单测（task04）
...
describe("重建索引", () => {
  it("POST /collections/rebuild body {collection_name, partition_name, mode} → 202 job_id", async () => {
    mockPost.mockResolvedValueOnce({
      data: { job_id: "r_abc123", accepted: true, message: "已接受重建", estimated_rows: 4969 },
    });
...
describe("参数预设", () => {
  it("GET /presets 返回列表", async () => {
    mockGet.mockResolvedValueOnce({
      data: [{ id: 1, preset_name: "平衡-default", is_default: true, top_k: 20, final_max_k: 6, cutoff_drop_ratio: 0.4, rrf_k: 60, use_hyde: true, enable_graph: true, llm_model_pref: "fast", created_at: "", updated_at: "" }],
    });
  ...
  it("POST /presets is_default=true 透传（后端事务置 0 旧默认）", async () => {
    mockPost.mockResolvedValueOnce({
      data: { id: 3, preset_name: "新默认", is_default: true, top_k: 20, final_max_k: 6, cutoff_drop_ratio: 0.4, rrf_k: 60, use_hyde: true, enable_graph: true, llm_model_pref: "quality", created_at: "", updated_at: "" },
    });
...
describe("审计日志", () => {
  ...
  it("空过滤只传 page/page_size", async () => {
    mockGet.mockResolvedValueOnce({ data: { page: 1, page_size: 20, total: 0, items: [] } });
    await listRagAuditLog({});
    expect(mockGet).toHaveBeenCalledWith("/api/admin/rag/audit-log", {
...
describe("高级检索", () => {
  ...
  it("tenant_ids/content_types 数组透传", async () => {
    mockPost.mockResolvedValueOnce({
      data: { docs: [], graph_entities: [], retrieved_count: 0, final_count: 0, degraded_reason: "MILVUS_UNAVAILABLE" },
    });
