/**
 * lib/api/admin/mcp.ts 单测（task04）
...
describe("import-url", () => {
  it("POST /servers/import-url body {url}", async () => {
    mockPost.mockResolvedValueOnce({
      data: { imported: true, server_id: 5, server_code: "stdio-import-1", display_name: "Import", transport: "stdio", discovered_tool_count: 4, warning: null },
    });
...
describe("工具列表 / discover", () => {
  it("GET /servers/{id}/tools 分页", async () => {
    mockGet.mockResolvedValueOnce({
      data: { total: 1, page: 1, page_size: 20, items: [{ id: 2, server_id: 1, tool_name: "add", display_name: "整数加法", description: "a+b", input_schema: { type: "object" }, category: "data", yn: 1, discovered_at: "" }] },
    });
  ...
  it("GET /servers/{id}/discover-live 直连实时快照", async () => {
    mockGet.mockResolvedValueOnce({
      data: { server_id: 1, ok: true, tool_count: 4, tools: [{ tool_name: "ping", display_name: "ping", description: "pong", input_schema: {}, category: "" }], reason: null, latency_ms: 109 },
    });
...
describe("工具测试", () => {
  it("POST /api/mcp/tools/test body {tool_id, args}（args 是 dict 非字符串）", async () => {
    mockPost.mockResolvedValueOnce({
      data: { status: "SUCCESS", result: { sum: 3, was_negative: false }, error_message: null, latency_ms: 94, call_id: "mcp-1", server_id: 1, tool_name: "add", content_text: '{"sum": 3}' },
    });
  ...
  it("无 tool_id 时按 server_id + tool_name 调用", async () => {
    mockPost.mockResolvedValueOnce({
      data: { status: "SUCCESS", result: null, error_message: null, latency_ms: 10, call_id: "mcp-2", server_id: 1, tool_name: "ping", content_text: null },
    });
...
describe("调用日志", () => {
  it("server_id/tool_name/status/user_id 过滤 + 分页", async () => {
    mockGet.mockResolvedValueOnce({ data: { total: 0, page: 1, page_size: 10, items: [] } });
...
describe("健康扫描 / 注销", () => {
  it("POST /health-scan 返回扫描汇总", async () => {
    mockPost.mockResolvedValueOnce({
      data: { scanned: 3, ok_count: 2, error_count: 1, items: [], elapsed_ms: 42 },
    });
  ...
  it("DELETE /servers/{id} 软删注销", async () => {
    mockDelete.mockResolvedValueOnce({ data: { ok: true, server_id: 3, deleted: true } });
