...
describe("getProgressDashboard", () => {
  it("拼装 /api/progress/dashboard?days=14（默认）", async () => {
    mockGet.mockResolvedValueOnce({ data: makeDashboard() });
...
describe("getProgressCourses（严格封装，不吞错）", () => {
  it("成功返回数组", async () => {
    mockGet.mockResolvedValueOnce({
      data: [{ series_id: 1, series_title: "a", overall_ratio: 0.5, total_sessions: 4, completed_sessions: 2 }],
    });
...
describe("getMySubjectPreferences（snake/camel 双命名兜底）", () => {
  it("主源：/me/profile 结构化 subject_preferences 直接返回，不再请求 /me", async () => {
    mockGet.mockResolvedValueOnce({
      data: { subject_preferences: [{ subject_code: "english", preference_score: 5 }] },
    });
  ...
  it("主源 subject_preferences 空数组 → 返回 []（合法空偏好，不发降级请求）", async () => {
    mockGet.mockResolvedValueOnce({ data: { subject_preferences: [] } });
    const prefs = await getMySubjectPreferences();
  ...
  it("降级：主源无字段 → /me 的 subjectPreferences: string[] → preference_score=5", async () => {
    mockGet
      .mockResolvedValueOnce({ data: { nickname: "x" } }) // /me/profile 无 subject_preferences
      .mockResolvedValueOnce({ data: { subjectPreferences: ["english", "math"] } }); // /me
    const prefs = await getMySubjectPreferences();
    expect(prefs).toEqual([
      { subject_code: "english", preference_score: 5 },
  ...
  it("降级：/me 的 profile.subject_preferences 结构化也兜底", async () => {
    mockGet
      ...
      .mockResolvedValueOnce({
        data: { profile: { subject_preferences: [{ subject_code: "physics", preference_score: 4 }] } },
      });
    const prefs = await getMySubjectPreferences();
