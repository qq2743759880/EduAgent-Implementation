...
export function getProgressDashboard(days = 14): Promise<DashboardOut> {
  return http.get<DashboardOut>(`/api/progress/dashboard?days=${days}`);
}

/** 严格封装：错误向上抛；不复用 learning.ts getMyCourses()（其 Array.isArray ? : [] 兜底吞错） */
export function getProgressCourses(): Promise<ProgressCourseItem[]> {
  return http.get<ProgressCourseItem[]>("/api/progress/courses");
}

/**
 * 学科偏好（雷达派生输入）。按端点契约读取（非字段别名兜底）：
 *   主源 = GET /api/users/me/profile → subject_preferences（UserProfile.snake 权威）；
 *   降级 = GET /api/users/me → subjectPreferences: string[]（该端点契约为 camelCase
 *          code 数组）→ preference_score=5（对齐 PUT 语义），me.profile 嵌套为 snake。
 * 主源字段存在（即使空数组）时不再发降级请求。
 */
export async function getMySubjectPreferences(): Promise<SubjectPreference[]> {
  const profile = await http.get<Record<string, unknown>>("/api/users/me/profile");
  const fromProfile = readProfileSubjectPreferences(profile);
  if (fromProfile) return fromProfile;
  const me = await http.get<Record<string, unknown>>("/api/users/me");
  return readMeSubjectPreferences(me) ?? [];
}

/** /api/users/me/profile 契约：subject_preferences（UserProfile） */
function readProfileSubjectPreferences(
...
function readMeSubjectPreferences(
  ...
  if (nested && typeof nested === "object" && !Array.isArray(nested)) {
    return normalizeStructured((nested as Record<string, unknown>).subject_preferences);
  }
  return null;
}

/** [{subject_code, preference_score}] 结构化形态（preference_score 缺省按 5） */
function normalizeStructured(raw: unknown): SubjectPreference[] | null {
