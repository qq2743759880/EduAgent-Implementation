...
describe("listPosts", () => {
  it("默认参数：sort=HOT&page=1&page_size=20", async () => {
    mockGet.mockResolvedValueOnce({
      data: { total: 0, page: 1, page_size: 20, items: [], mine_total_posts: 0 },
    });
    await listPosts();
...
describe("createPost（写操作）", () => {
  it("body 字段对齐后端 schema：title/content_md/board_code/tags", async () => {
    mockPost.mockResolvedValueOnce({
      data: { post_id: 42, points_awarded: 5, badge_unlocked: [] },
    });
...
describe("点赞 / 收藏 / 回帖（写操作）", () => {
  it("togglePostLike 走 POST /posts/{id}/like", async () => {
    mockPost.mockResolvedValueOnce({
      data: { target_type: "POST", target_id: 7, react_type: "LIKE", active: true, total_count: 3, points_awarded: 0 },
    });
  ...
  it("togglePostFavorite 走 POST /posts/{id}/favorite", async () => {
    mockPost.mockResolvedValueOnce({ data: { active: false, total_count: 0 } });
  ...
  it("createComment body 携带 content_md + parent_id，响应含 points=2", async () => {
    mockPost.mockResolvedValueOnce({
      data: { comment_id: 9, created_at: "2026-08-12T10:00:00", points: 2 },
    });
...
describe("comments / rankings（列表）", () => {
  ...
  it("getRankings 拼装 scope/dimension/top_n（契约：DAILY|WEEKLY|MONTHLY|ALL_TIME）", async () => {
    mockGet.mockResolvedValueOnce({ data: { scope: "WEEKLY", dimension: "POINTS", top: [], my_rank: null } });
