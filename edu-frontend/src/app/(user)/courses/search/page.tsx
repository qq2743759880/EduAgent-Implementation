...
function CoursesSearchPageInner() {
  ...
  const filters: SubjectLevelFiltersValue = {
    q: parsed.q,
    category: parsed.category as SubjectLevelFiltersValue["category"],
  ...
  const { data, isLoading, isError, error, refetch } = useQuery({
    ...
    async queryFn() {
      const category = filters.category !== "all"
        ? CATEGORY_OPTIONS.find((c) => c.code === filters.category)?.name
        : undefined;
      return listSeries({
        category,
        keyword: filters.q,
  ...
  /* 兼容 filters 与 URL 不同步（本地表单未 submit）时，不重复刷新
     依赖用稳定序列（JSON 字符串）避免 filters 对象引用变化导致每次渲染都同步 */
          ...
          {appliedChips.length > 0 && (
            <div className="mt-4 flex flex-wrap items-center gap-2">
              ...
              <Button
                ...
                onClick={() => pushUrl({ category: "all", q: undefined, min_price: undefined, max_price: undefined, page: 1 })}
              >
                清空全部
              </Button>
            </div>
        ...
        <div className="grid gap-6 lg:grid-cols-[320px_minmax(0,1fr)]">
          ...
          <section className="space-y-6">
            {isLoading ? (
              ...
              <EmptyState onClear={() => pushUrl({ category: "all", q: undefined, min_price: undefined, max_price: undefined, page: 1 })} />
            ) : (
              <>
...
function buildChips(
  ...
  if (filters.category !== "all") {
    const cat = CATEGORY_OPTIONS.find((s) => s.code === filters.category);
    if (cat) {
      chips.push({
        label: `分类：${cat.name}`,
        clear: { category: "all", page: 1 },
      });
    }
  }
