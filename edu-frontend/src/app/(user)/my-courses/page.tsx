    ...
    <div className="mx-auto w-full max-w-7xl px-4 py-6 md:px-6 md:py-10">
      <header className="mb-6 flex flex-wrap items-end justify-between gap-4">
        <div className="space-y-1.5">
          ...
          <h1 className="text-2xl font-bold tracking-tight md:text-3xl">我的课程</h1>
...
function StatStrip() {
  const items = [
    { key: "in_progress", label: "进行中", Icon: GraduationCap, tone: "text-sky-600", bg: "bg-sky-500/10" },
    { key: "completed", label: "已完成", Icon: Trophy, tone: "text-emerald-600", bg: "bg-emerald-500/10" },
    { key: "favorited", label: "已收藏", Icon: Heart, tone: "text-rose-600", bg: "bg-rose-500/10" },
    { key: "faq", label: "需要帮助？", Icon: CircleHelp, tone: "text-indigo-600", bg: "bg-indigo-500/10" },
  ] as const;
  return (
    <div className="grid grid-cols-2 gap-2 md:grid-cols-4">
      {items.map(({ key, label, Icon, tone, bg }) => (
        <div
          key={key}
          className="flex items-center gap-3 rounded-2xl border bg-white/60 px-3 py-2.5 shadow-sm backdrop-blur-sm"
        >
