...
const OUT = process.argv[3] || "page-verify-w2";
const PORT = 9226, BASE = "http://127.0.0.1:3000", API = "http://127.0.0.1:8000";
...
const PAGES = [["coupons.html", "coupons"], ["chat.html", "chat"], ["community.html", "community"],
  ["achievements.html", "achievements"], ["dashboard.html", "dashboard"]];
for (const [route, name] of PAGES) {
  errs.length = 0;
  await send("Page.navigate", { url: `${BASE}/${route}` });
  await sleep(3500);
  const r = await send("Runtime.evaluate", { expression: `(() => ({h1:(document.querySelector("h1")||{}).textContent||"",
