...
- ran_at: 2026-09-14T21:56:43  |  git_rev: `57491e5`  |  seed: 20260914
...
- **docs Jaccard**: mean=0.6411, p50=0.6667, p95=1.0, min=0.0, max=1.0（五条件参考 mean>0.9；=1 共 47 条，<0.5 共 43 条，=0 共 7 条）
...
|---|---|---|---|---|---|
| 旧 run_agent_turn | 100 | 3.412 | 4.103 | 0.209 | 13.774 |
| 新 run_agent(图) | 100 | 3.512 | 4.513 | 0.276 | 13.891 |

- P95 容差对照（五条件参考：新 ≤ 旧 P95 × 1.1）: 旧 4.103s vs 新 4.513s

## 确定性设计（详档 v1.2 A1 承接）

- RULE_ROUTING_ENABLED=True / LLM_TEMPERATURE=0.0 / USE_AGENT_LOOP=True 启动断言通过（生产默认即确定性配置；HyDE 为纯字符串改写无 LLM；检索层零采样噪声）。
- 每样本同 query 双轮 intent 预验（旧=decide_agent_plan；新=route_node 单节点）；不稳定样本 0 条。
- 答案语义 30% 抽检（仅入报告）：n_done=30, equivalent=27, not_equivalent=1, judge_failed=2
...
| intent分歧<5% | {"当前值": "0.0%"} |
| docsJaccard>0.9 | {"当前值": 0.6411} |
| P95TTFT容差 | {"旧P95_s": 4.103, "新P95_s": 4.513, "阈值说明": "旧路径实测 P95 + 10%；阈值裁定属用户/编排者，本探针只报数"} |

## 已知口径与偏差（批判性审查输入）

1. eval_set32 现存文件为 100 条历史产物（recall_topk=300），本探针取 cases[:32] 确定性切片；R20min 重生成 32 条后复跑即对齐详档口径。
