...
NEXTJS_OPS_ENDPOINTS = frozenset({
    ...
    ("POST", "/payment-notifications/channel"),         # 真实渠道回调（服务端对服务端，前端永不接入）— R22PAY 062704a/tracker 346；canonical 同步（W-NEXT-FEBE-CANON-SYNC-001，原 TEST-BASE tests 侧 overlay 移交），冻结条目见 contracts/reshape-r-health.json G3_trade_payment（2026-09-19 RESHAPEB-MOVE 归位，原 reshape-b.json） PAY-GATE-346 amendment
})
