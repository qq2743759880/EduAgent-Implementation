...
class HomeworkSubmitIn(BaseModel):
    """作业提交请求（一课次一次作业）。"""
    session_id: int = Field(..., gt=0, description="课次 ID = curriculum_session.id，也即 homework_id")
    answers: list[SubmittedAnswerIn] = Field(..., min_length=1, max_length=100, description="所有作答")


class ExamSubmitIn(BaseModel):
    """考试提交请求。"""
    paper_id: int = Field(..., gt=0, description="试卷 ID = admin_exam_paper.id，也即 exam_id")
