...
# ------------------------------------------------------------
# tool 强信号：数值计算 / 换算 / 格式化 / 写类工具操作（明确可调用工具）
# ------------------------------------------------------------
_TOOL_PATTERNS: tuple[re.Pattern[str], ...] = (
    ...
    # W-NEXT-WRITE1（CR-WRITETOOLS-001）：写类操作意图（favorite_add 本人收藏）。
    # 误路由安全边际：仅改路由不改执行——是否真执行仍由 _parse_heuristic 的
    # 「显式 series_id」门槛把关（「怎么收藏课程？」无 ID → 零计划 → 正常 RAG 回答）。
    re.compile(r"收藏|取消收藏|加入收藏"),
    # AUTO20 T12（CR-WRITETOOLS-001 第二批）：建课意图（course_create，admin_write 高危写）。
    # 误路由安全边际同上：仅改路由不改执行——真触发仍由 _parse_heuristic 的
    # 「标题+系列码同现」门槛把关（「怎么创建课程？」无参 → 零计划 → 正常 RAG 回答）。
    re.compile(r"创建.{0,6}(课程|系列)|建课|新课|新增.{0,4}课程"),
)
