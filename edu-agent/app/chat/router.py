...
async def sessions_create(
    ...
    try:
        return await service_create_session(user.user_id, body)
...
async def sessions_list(
    ...
    try:
        return await list_sessions(user.user_id, user.role, limit=limit)
    except Exception as e:
        _translate_exception(e)
...
# 3. 非流式问答
# ============================================================
@router.post("", response_model=RagAnswerResponse)
async def chat_non_stream(
    ...
    try:
        return await service_chat_answer(req, user_id=user.user_id, role=user.role)
    except Exception as e:
        _translate_exception(e)
...
async def chat_stream_sse(
    ...
    async def _gen() -> AsyncGenerator[bytes, None]:
        ...
        yield _sse_line(SseEventType.DONE.value, final_info)
