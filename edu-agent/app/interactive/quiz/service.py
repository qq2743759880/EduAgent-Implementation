...
async def _load_by_code_or_id(*, code: str | None = None, qid: int | None = None) -> Question | None:
    ...
    # admin_question 真实题（若存在）
    if qid:
        try:
            row = await fetch_one(
                "SELECT id, subject_code, question_type, title, options_json, answer_json, score_max, "
                "explain_text, knowledge_ids_json FROM admin_question WHERE id=%s AND yn=1",
                (qid,),
            )
        except Exception as e:
            # MySQL 1146 / ProgrammingError: Table 'edu.admin_question' doesn't exist
            # 表未初始化时直接视为「该 qid 无对应题目」，交由上层 fallback；
            # 避免把「表未建好」这种 DB 问题直接抛成用户侧 500 红屏。
            msg = str(e).lower()
            if ("doesn't exist" in msg or "1146" in msg or "no such table" in msg):
                ...
                raise
        if row:
            opt = _unjson(row["options_json"]) or []
            corr = _unjson(row["answer_json"])
            return Question(
                question_id=int(row["id"]),
                custom_code=f"AQ-{row['id']}",
                subject_code=row["subject_code"] or "general",
                question_type=row["question_type"] or SINGLE,
                title=row["title"] or "",
                score_max=float(row["score_max"] or 5),
                choices=[Choice(**x) for x in opt if isinstance(x, dict)],
                correct=corr,
                explain_template=row["explain_text"],
                knowledge_codes=[str(x) for x in (_unjson(row["knowledge_ids_json"]) or [])],
            )
    return None
