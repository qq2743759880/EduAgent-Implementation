...
async def create_comment(user_id: int, display_name: str | None, post_id: int,
    ...
    if post is None:
        raise ResourceNotFoundError(f"帖子不存在 {post_id}", code=COMMUNITY_POST_NOT_FOUND)
    if bool(post.get("is_locked")):
        raise BadRequestError("帖子已锁定，不可评论", code=COMMUNITY_POST_LOCKED)
    ...
    if int(row.get("author_id") or 0) != user_id:
        ...
        if payload.is_pinned is None and payload.is_locked is None:
            raise PermissionDeniedError(
                "不能修改他人的帖子",
                code=COMMUNITY_FORBIDDEN_UPDATE,
            )
    sets: list[str] = []
    args: list[Any] = []
