...
async def award_points(user_id: int, biz_key: str, point_type: str, delta: int,
    ...
    try:
        ...
        return 0
    # 排行写入侧：积分变更实时增量进 Redis ZSET（1s 内可查；失败静默降级）
    await _zincr_points_zset(user_id, delta, datetime.utcnow())
    return delta
...
# ========== 5. 排行榜（Redis ZSET 优先：周榜快照 + 实时增量兜底）==========
SCOPE_ORDER = ["DAILY", "WEEKLY", "MONTHLY", "ALL_TIME"]
DIM_ORDER = ["POINTS", "STUDY_MIN", "BADGE_COUNT"]

# 各 scope 的 ZSET key TTL（秒）：周期结束再多留几天防空窗；ALL_TIME 近似长期
_ZSCOPE_TTL = {
    "DAILY": 2 * 86400,
    "WEEKLY": 8 * 86400,
    "MONTHLY": 33 * 86400,
...
def _rank_zset_key(scope: str, dimension: str, now: datetime) -> str:
    """ZSET key = g-rank:{scope}:{dimension}:{period}。

    period 用于区分不同周期（当天/当周周一/当月/ALL_TIME），避免跨周期数据互相污染。
    与 _scope_start 保持同一口径，保证写入侧（zincrby）与读取侧（zrevrange）落在同一 key。
    """
    start = _scope_start(scope, now=now)
    if scope == "DAILY":
        period = start.strftime("%Y-%m-%d")
    elif scope == "WEEKLY":
        period = start.strftime("%Y-W%W")
    elif scope == "MONTHLY":
        period = start.strftime("%Y-%m")
    else:  # ALL_TIME
        period = "ALL"
    return f"g-rank:{scope}:{dimension}:{period}"


async def _zincr_points_zset(user_id: int, delta: int, now: datetime) -> None:
    """写入侧：给当前周期所有 scope 的 POINTS ZSET 增量累加积分（实时增量，1s 内可查）。

    任何异常都静默降级（不影响主流程积分落库）——Redis 不可用不阻塞业务。
    """
    try:
    ...
    for scope in SCOPE_ORDER:
        ...
        try:
            await r.zincrby(key, delta, str(user_id))
...
async def _ranking_from_zset(scope: str, dimension: str, top_n: int, now: datetime) -> list[dict] | None:
    ...
    返回 list[{rank_no,user_id,metric_value}]；Redis 不可用返回 None 由调用方走 SQL 兜底。
    """
    try:
        from app.database import get_redis
        r = get_redis()
        ...
        return None
    key = _rank_zset_key(scope, dimension, now)
    try:
        rows = await r.zrevrange(key, 0, top_n - 1, withscores=True)
    except Exception:
        return None
    if rows:
        return [
            {"rank_no": i, "user_id": int(member), "metric_value": int(score)}
            for i, (member, score) in enumerate(rows, start=1)
        ]
    # 空：回填快照（用 SQL 实时算并写入 ZSET，后续都走 ZSET）
    try:
        live = await _live_ranking(scope=scope, dimension=dimension, top_n=top_n)
        if live:
            await r.zadd(key, {str(row["user_id"]): row["metric_value"] for row in live})
            await r.expire(key, _ZSCOPE_TTL[scope])
        return live
    except Exception:
        return None
...
async def ranking(user_id: int, scope: str = "DAILY", dimension: str = "POINTS",
    ...
    # 优先 ZSET（快照命中）→ 无则 SQL 实时兜底
    snap = await _ranking_from_zset(scope, dimension, top_n, now_dt)
    ...
    # 昵称（ZSET 模式无 name，需补查；SQL 实时兜底也无 name，保持一致）
    name_map: dict[int, str] = {}
    if top_uid:
        ph = ",".join(["%s"] * len(top_uid))
        nm = await fetch_all(
            f"SELECT id, COALESCE(nickname, real_name, account, '') AS name FROM sys_user WHERE id IN ({ph})",
            tuple(top_uid),
        )
        for x in nm:
            name_map[int(x["id"])] = x.get("name") or f"用户{x['id']}"
    ...
    for r in snap:
        ...
        top_list.append(RankingRow(
            ...
            user_id=uid,
            user_name=name_map.get(uid) or r.get("name") or f"用户{uid}",
    ...
    # 我的排名：在 top_list 里找，否则插值计算（ZSET/实时同逻辑）
...
def _scope_start(scope: str, now: datetime | None = None) -> datetime:
    if now is None:
        now = datetime.utcnow()
