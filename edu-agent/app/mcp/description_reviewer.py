...
# 体检结果只读缓存 TTL（task97 GWT①：同参结果 60s 内命中）
REVIEW_CACHE_TTL = 60


def _stable_summary_sentence(description: str) -> str:
    """取描述首句作为「稳定摘要句」——描述被 FAST 重写后，前缀摘要仍锚定原首句，key 不变。

    用于把重写后的完整描述存进延迟加载层时，保持其摘要列表 key 稳定（task97 GWT①）。
    """
    from .deferred import _first_sentence

    return _first_sentence(description)


def apply_rewrite_to_deferred(index, tool_meta: Mapping[str, Any], rewritten_text: str) -> None:
    """把 FAST 重写后的描述「存进延迟加载层」，且保持摘要列表 key 稳定。

    - summary_sentence 锚定原描述的稳定首句 → 重写不改变前缀摘要；
    - description 存为重写后全文（决策 LLM 选中工具后按需拉取完整定义）；
    - 返回后 index.prefix_key() 与重写前一致（deferred 吸收重写，R3 生效）。
    """
    from .deferred import DeferredToolIndex

    if not isinstance(index, DeferredToolIndex):
        return
    original = str(tool_meta.get("description") or "")
    meta = dict(tool_meta)
    meta["description"] = rewritten_text
    # 关键：锚定原首句，保证前缀 key 不变
    meta["summary_sentence"] = _stable_summary_sentence(original) or _stable_summary_sentence(rewritten_text)
    meta["admin_only"] = bool(tool_meta.get("admin_only"))
    index.set_tools([meta])
...
async def run_description_review(
    ...
    task97 GWT①：返回结果走 60s 只读缓存（get_or_load）——同一 server + 工具描述指纹不变时，
    60s 内的重复点击直接命中缓存，避免重复 FAST 重写 / 审计落库。描纹变化（描述或 schema 变了）
    则自动失效重算。
    """
    from . import registry

    rows = await registry.list_tools_for_review(server_id=server_id)
    # 描纹：server_id + 每个工具的 (id, 描述 hash, schema hash) —— 任一描述变更即失效
    fp = hashlib.md5(
        repr([
            (int(r["id"]), hash(str(r.get("description") or "")), hash(str(r.get("input_schema") or "")))
            for r in rows
        ]).encode("utf-8")
    ).hexdigest()
    cache_key = f"mcp:desc-review:{server_id}:{fp}"

    async def _do_review() -> dict:
        reviewed: list[dict] = []
        low_count = 0
        rewritten_count = 0
    ...
    try:
        ...
        result = await get_or_load(cache_key, _do_review, ttl=REVIEW_CACHE_TTL)
        if isinstance(result, dict) and "cache_hit" not in result:
            result["cache_hit"] = True
        return result
    except Exception:
        # Redis 不可用时退化为直接计算（缓存降级，不阻断体检）
