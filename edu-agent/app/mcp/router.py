...
# ============================================================
@router.post("/servers", response_model=MCPServerDetail,
...
async def p8_register_server(payload: MCPServerCreate, me: CurrentUser = Depends(get_current_user)):
    try:
        return await registry.register_server(created_by=int(me.user_id), payload=payload)
    except AppException as exc:
        raise HTTPException(status_code=exc.http_status, detail=exc.message)


@router.get("/servers", response_model=MCPServerListResp,
...
async def p8_server_detail(server_id: int):
    try:
        return await registry.get_server_detail(server_id)
    except AppException as exc:
        raise HTTPException(status_code=exc.http_status, detail=exc.message)
...
async def p8_update_server(server_id: int, payload: MCPServerUpdate):
    try:
        return await registry.update_server(server_id, payload)
    except AppException as exc:
        raise HTTPException(status_code=exc.http_status, detail=exc.message)
...
async def p8_delete_server(server_id: int, me: CurrentUser = Depends(get_current_user)):
    try:
        ...
        return {"ok": True, "server_id": server_id, "deleted": True}
    except AppException as exc:
        raise HTTPException(status_code=exc.http_status, detail=exc.message)
...
async def p8_health_check(server_id: int):
    try:
        return {"server_id": server_id, **(await executor.health_check_server(server_id))}
    except AppException as exc:
        raise HTTPException(status_code=exc.http_status, detail=exc.message)
...
# ============================================================
@router.post("/servers/import-url", response_model=MCPImportResp,
...
# ============================================================
@router.get("/servers/{server_id}/tools", response_model=MCPToolListResp,
...
async def p8_list_tools_of_server(
    ...
    return await registry.list_tools(server_id=server_id, category=category, keyword=keyword,
                                     page=page, page_size=page_size)


@router.get("/tools", response_model=MCPToolListResp,
...
async def p8_list_tools_global(
    ...
    return await registry.list_tools(server_id=None, server_code=server_code, category=category,
...
# ============================================================
@router.post("/tools/test", response_model=MCPToolTestResp,
...
# ============================================================
@router.get("/call-log", response_model=MCPLogListResp,
...
async def p8_list_call_log(
    ...
    return MCPLogListResp(total=total, page=page, page_size=page_size, items=items)


@router.get("/call-log/{log_id}", response_model=MCPLogDetail,
...
# 7. 管理端调试面板专用 API
# ============================================================
@router.get("/servers/{server_id}/discover-live", response_model=MCPLiveDiscoverResp,
...
async def p8_live_discover(server_id: int):
    ...
    return MCPLiveDiscoverResp.model_validate(result)


@router.post("/servers/{server_id}/raw-rpc", response_model=MCPRawRpcResp,
...
async def p8_raw_rpc(server_id: int, payload: MCPRawRpcReq):
    ...
    return MCPRawRpcResp.model_validate(result)


@router.post("/health-scan", response_model=MCPHealthScanResp,
             summary="P8-15【调试】对所有 yn=1 的 MCP Server 做批量健康扫描")
async def p8_health_scan():
    result = await executor.scan_all_servers_health()
    return MCPHealthScanResp.model_validate(result)


# ============================================================
# 7B. stdio 长连接会话（Checkpoint）：创建/列表/详情/保活/关闭 + GC
# ============================================================
@router.post("/sessions", response_model=MCPSessionResp,
...
async def p8_sessions_create(payload: MCPSessionCreateReq,
    ...
    return MCPSessionResp.model_validate(result)


@router.get("/sessions", response_model=MCPSessionListResp,
...
async def p8_sessions_list():
    ...
    return MCPSessionListResp(total=len(items), items=items)


@router.get("/sessions/{session_id}", response_model=MCPSessionResp,
            summary="P8-19【调试】单个会话快照（不存在 404）")
async def p8_sessions_get(session_id: str):
    item = await executor.session_get(session_id)
    if item is None:
        return MCPSessionResp(ok=False, message=f"session={session_id} not found", session=None)
    return MCPSessionResp(ok=True, message="ok", session=MCPSessionItem.model_validate(item))


@router.post("/sessions/{session_id}/touch", response_model=MCPSessionResp,
...
async def p8_sessions_touch(session_id: str):
    ...
    if not result.get("ok"):
        return MCPSessionResp(ok=False, message=str(result.get("message") or "touch fail"), session=None)
    return MCPSessionResp(ok=True, message=str(result.get("message") or "ok"),
                          session=MCPSessionItem.model_validate(result["session"]))


@router.delete("/sessions/{session_id}", response_model=MCPSessionResp,
...
async def p8_sessions_close(session_id: str):
    ...
    if (result.get("session") or None) is None:
        return MCPSessionResp(ok=True, message=str(result.get("message") or "closed"), session=None)
    return MCPSessionResp(ok=True, message=str(result.get("message") or "closed"),
                          session=MCPSessionItem.model_validate(result["session"]))
