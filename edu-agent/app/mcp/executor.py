...
from app.database import execute_write, fetch_one
...
async def call_tool(*,
    ...
    async def _run_once() -> MCPToolTestResp:
        ...
        status_eff, error_message, result_parsed, content_text = await _invoke_transport(
            server, tool_name_eff, args, call_id)
        latency_ms = int((time.perf_counter() - t0) * 1000)
        await _write_call_log(
            call_id=call_id, server_id=server_id_eff, tool_name=tool_name_eff,
        ...
        # task33 GWT②：向 per-server 熔断器上报结果（失败以状态码表达，故显式 record）
        try:
        ...
        return MCPToolTestResp(
            ...
            server_id=server_id_eff, tool_name=tool_name_eff,
