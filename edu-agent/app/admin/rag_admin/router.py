...
async def collections_list(user: AdminOnly):
    try:
        return ok(await list_collections())
    except Exception as e:
        _translate(e)


@router.post("/collections/rebuild", response_model=dict, status_code=status.HTTP_202_ACCEPTED)
async def collections_rebuild(body: CollectionRebuildRequest, user: AdminOnly):
    try:
        return ok(await rebuild_collection(body, operator_user_id=int(user.user_id)))
    except Exception as e:
        _translate(e)
...
async def presets_list(user: AdminOnly):
    try:
        return ok(await list_presets())
    except Exception as e:
        _translate(e)


@router.post("/presets", response_model=dict, status_code=status.HTTP_201_CREATED)
async def presets_create(body: ParamPresetCreate, user: AdminOnly):
    try:
        return ok(await create_preset(body, operator_user_id=int(user.user_id)))
    except Exception as e:
        _translate(e)
...
async def audit_log_list(
    ...
    try:
        return ok(await list_audit_log(
...
async def admin_search_endpoint(body: AdminSearchRequest, user: AdminOnly):
    try:
        return ok(await admin_search(body))
    except Exception as e:
        _translate(e)
