...
def encode_dense_batch(texts: list[str]) -> list[list[float]]:
    ...
    策略（2026-08-22 用户裁定云端优先）：
      · texts 全为空 → 直接返回全零向量（避免 API 报错）
      · EMBED_BACKEND="cloud"（默认）→ 优先 OpenAI 兼容 Embedding API（火山/硅基/DashScope，
        按 EMBEDDING_* 配置），失败回退本地 BGE-M3 CUDA
      · EMBED_BACKEND="cuda" → 本地 BGE-M3 优先，失败回退 API
    ...
    def _try_local_bge() -> list[list[float]] | None:
        bge = _get_bge_model()
        if bge is None:
            return None
        try:
    ...
    def _try_api() -> list[list[float]] | None:
        try:
