...
## 步骤
1. **跑生成器**:Milvus 在跑前提下执行 build_eval_set32.py,产出 32 条 {query, doc_id(=golden chunk_id), **doc_sha256(content 哈希)**}。抽检 5 条:query 确实可由该 chunk 回答(打印人工可读对照入报告);不合理的替换并记录。
...
## GWT(机验)
①eval_set32.json 产出且含双键 golden;②重跑两次指标一致;③nprobe=1 灵敏度实验指标下降;④基线+阈值文件入库。
