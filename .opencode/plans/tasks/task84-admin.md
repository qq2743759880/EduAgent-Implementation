...
- agent 链：同上完整流水线(编辑器用 react-markdown)
- MCP：context7(Next16)；playwright
- 前端任务必须先过 HTML 原型审核流（产出 test-reports/fe-html/task84-*.html → 用户审核给图 → 返工 → 签收 APPROVED → 写 React）
...
- 完成动作：写完工报告 test-reports/task84-completion-report.md（按 task-report-template.md）→ git commit → sync.ps1
